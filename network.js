/* ═══════════════════════════════════════════════════════════════
   どれトレ ネットワーク定義(路線図)
   どれトレ本体(index.html)とインプットツール(timetable-input-tool.html)の
   両方がこのファイルを読み込む。路線・駅を追加する場合はここだけを編集する。

   確実対応スコープ:
     相鉄線(海老名)・相鉄いずみ野線(湘南台) → 新横浜 → 日吉 → 武蔵小杉 → 田園調布
     → 目黒線 → 白金高輪 → 南北線・埼玉高速鉄道(浦和美園) / 都営三田線(西高島平)
     東横線は新横浜〜田園調布の並走区間のみ(渋谷方面は準備段階)
   準備段階(prep): 東横線渋谷方面などの分岐は駅リストだけ保持し、拡張の余地を残す

   segments: 駅の並び [駅名, 起点からの累積所要分(各停・概算)]
   ※所要分は実データが無い区間の概算値。実データはtrains-input.json側が持つ。
   ═══════════════════════════════════════════════════════════════ */
const NETWORK = {
  schemaVersion: 1,
  ...
  segments: {
    /* ── 相鉄線(西側) ── */
    SOTETSU_MAIN:    [["海老名",0],["かしわ台",3],["さがみ野",5],["相模大塚",7],["大和",9],["瀬谷",12],["三ツ境",14],["希望ヶ丘",16],["二俣川",19]],
    SOTETSU_IZUMINO: [["湘南台",0],["ゆめが丘",2],["いずみ中央",4],["いずみ野",6],["弥生台",8],["緑園都市",10],["南万騎が原",12],["二俣川",15]],
    SOTETSU_SHINYOKO:[["二俣川",0],["鶴ケ峰",2],["西谷",5],["羽沢横浜国大",8],["新横浜",12]],

    /* ── 東急新横浜線・並走区間 ── */
    SH:  [["新横浜",0],["新綱島",3],["日吉",6]],
    COR: [["日吉",0],["元住吉",2],["武蔵小杉",4],["新丸子",5],["多摩川",7],["田園調布",9]], // 東横線・目黒線 並走区間(全駅対面乗換)

    /* ── 目黒線系統(東側) ── */
    MG:  [["田園調布",0],["奥沢",2],["大岡山",4],["洗足",6],["西小山",7],["武蔵小山",9],["不動前",11],["目黒",13]],
    SHD: [["目黒",0],["白金台",2],["白金高輪",4]], // 三田線・南北線 共用区間
    ...
    /* ── 準備段階(prep): 現状は始発・終着ラベルとしてのみ使用。拡張時はprepを外す ── */
    TY_STUB: [["田園調布",0],["自由が丘",2],["都立大学",4],["学芸大学",6],["祐天寺",8],["中目黒",10],["代官山",11],["渋谷",14]]
  },

  /* 路線バッジ判定: 駅→路線コードの解決(共用区間は列車の系統で解決) */
  segmentLine: {
    SH:"SH",
    COR:{shared:{TY:"TY"}, default:"MG"},   // 東横線直通列車はTY、それ以外はMG
    MG:"MG",
  ...
  /* 系統定義: インプットツールの「系統」選択と方向判定の基準(東側の終端駅) */
  branches: {
  ...
  /* 駅選択リストのグループ表示(prep:true = 準備段階。時刻入力の対象外) */
  pickerGroups: [
...
/* 駅グラフ(隣接リスト)と距離・経路計算: 分岐を含む路線図でも機能する汎用処理 */
const NET_GRAPH = (function(){
  const g = new Map();
  function link(a,b){
  ...
  Object.values(NETWORK.segments).forEach(seg=>{
    for(let i=0;i<seg.length-1;i++) link(seg[i][0], seg[i+1][0]);
  });
  return g;
})();
...
function netBfsPath(from, to){
  ...
  while(queue.length){
    const cur = queue.shift();
    for(const nb of (NET_GRAPH.get(cur)||[])){
...
function netStationsOfGroups(includePrep){
  ...
  NETWORK.pickerGroups.forEach(gr=>{
